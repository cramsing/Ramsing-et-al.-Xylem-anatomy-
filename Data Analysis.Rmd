...
```
### PCA
```{r convert to zscores}
library(dplyr)
hist.zscores <- hist.charact %>% 
  mutate(diameter.zscore = (diameter - mean(diameter))/sd(diameter)) %>%
  mutate(density.zscore = (density - mean(density))/sd(density)) %>%
  mutate(surface.zscore = (surface - mean(surface))/sd(surface))
```



```{r histological PCA 1}
colnames(hist.zscores)
h.c.pca <- prcomp(hist.zscores[,c(17:19)], center = TRUE,scale. = TRUE)
...
# Pch.Dens.Error$`density class`<- as.character(Pch.Dens.Error$`density class`)
...
# Pch.S.Error$`surface class` <- as.character(Pch.S.Error$`surface class`)
...
# Pmin.D.Error$`diameter class` <- as.character(Pmin.D.Error$`diameter class`)
...
# Pmin.Dens.Error$`density class` <- as.character(Pmin.Dens.Error$`density class`)
...
PchConcplot <- x + geom_boxplot(color = "gray") + theme_classic() + theme(axis.text.x = element_text(size = 6, angle=90, hjust = 0.5), axis.title = element_text(size=8), axis.text = element_text(size = 8)) + theme(legend.position = "none") + labs(x = "Rootstock", 
       y = plab) +
...
pminplot <- ggplot(data = PminConc, aes(x = rootstock, y = Conc_pg.ul)) + geom_boxplot(color = "gray")
...
DensRplot <- DensR + theme_classic() + theme(axis.text.x = element_text(angle=90, hjust = 1)) +
  theme(legend.position = "none") + labs(x = "Rootstock variety", y = denslab) + scale_fill_grey() +  stat_compare_means(method = "anova", label.y = 100, label.x = "COUDERC 1202") +
...
SurfRplot <- SurfR + theme_classic() + theme(axis.text.x = element_text(angle=90, hjust = 1)) +
  theme(legend.position = "none") + labs(x = "Rootstock variety", y = surflab) + scale_fill_grey() +  
...
Pch.diam <- ggplot(PchConc, aes(x = diam.class, y = Conc_pg.ul, fill = diam.class)) + geom_boxplot() +
  labs(x = "Diameter class", y = plab) + scale_fill_viridis_d(name = "Diameter Class") + theme_classic() +
  stat_compare_means(method = "kruskal.test", label.y = 450, label.x = 1) +
  stat_compare_means(label = "p.signif", method = "wilcox.test",
                     ref.group = ".all.", hide.ns = TRUE)  
leg.diam <- get_legend(Pch.diam)
...
ggsave(filename = "Figure S3A.png", plot = pca.plot, device = "png", height = 5, width = 8,
       units = "in", dpi = 500)
pca.plot

library(ggplot2)
pca <- data.frame(h.c.pca$x[, 1:2]) # we only need the first two principal components
pca.plot2 <- ggplot(xxx, aes(y = PC1, x = PC2)) + geom_point(col = 'tomato2') + theme_classic()
ggsave(filename = "Figure S3B.png", plot = pca.plot2, device = "png", height = 5, width = 8,
       units = "in", dpi = 500)
pca.plot2
```
